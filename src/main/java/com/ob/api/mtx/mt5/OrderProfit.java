...
class OrderProfit {
    ...
    public final void Update(OrderData order, double bid, double ask) throws IOException {
        SymbolInfo sym = QC.symbols.GetInfo(order.getSymbol());
        UpdateSymbolTick(sym, bid, ask);
        if (sym.CalcMode == CalculationMode.Forex) {
            if (order.getOrderType() == OrderTypeData.Buy) {
                double lots = order.getLot().doubleValue() * sym.bid_tickvalue;
                var value = Math.round((QC.GetQuote(order.getSymbol()).Bid * lots) * Math.pow(10, sym.Digits)) / Math.pow(10, sym.Digits)
                            - Math.round((order.getPrice().doubleValue() * lots) * Math.pow(10, sym.Digits)) / Math.pow(10, sym.Digits);
                order.setProfit(BigDecimal.valueOf(value));
            } else {
                double lots = order.getLot().doubleValue() * sym.ask_tickvalue;
                var value = Math.round((order.getPrice().doubleValue() * lots)
                                       * Math.pow(10, sym.Digits)) / Math.pow(10, sym.Digits)
                            - Math.round((QC.GetQuote(order.getSymbol()).Ask * lots) * Math.pow(10, sym.Digits)) / Math.pow(10, sym.Digits);
                order.setProfit(BigDecimal.valueOf(value));
            }
        } else {
            double price = 0;
            if (order.getOrderType() == OrderTypeData.Buy) {
                price = (QC.GetQuote(order.getSymbol()).Bid - order.getPrice().doubleValue()) * sym.bid_tickvalue;
            } else {
                price = (order.getPrice().doubleValue() - QC.GetQuote(order.getSymbol()).Ask) * sym.ask_tickvalue;
            }
            order.setProfit(BigDecimal.valueOf(price * order.getLot().doubleValue()));
            if (sym.CalcMode == CalculationMode.Futures) {
                var profit = order.getProfit().doubleValue();
                profit /= sym.tick_size;
                order.setProfit(BigDecimal.valueOf(profit));
            }
        }
        order.setProfit(BigDecimal.valueOf(Math.round(order.getProfit().doubleValue() * Math.pow(10, sym.Digits)) / Math.pow(10, sym.Digits)));
    }
    ...
    private boolean UpdateSymbolTick(SymbolInfo sym, double bid, double ask) throws IOException {
        ...
        if (sym.CalcMode != CalculationMode.CFD) {
            sym.bid_tickvalue = 0;
            sym.ask_tickvalue = 0;
            return false;
        }
        double dTick = (sym.CalcMode == CalculationMode.Futures) ? sym.tick_value : sym.ContractSize;
