...
public class OrderSender {
    ...
    private void CryptRequestProtectKey(TradeRequest req, OutBuf buf) {
        if (Connection.ProtectKey == null) {
            if ((Connection.api.Account.TradeFlags & 8) != 0)
                throw new CodeException("Investor mode", Code.INVESTOR_PASSWORD);
            else {
                try {
                    KeyManagerFactory kmf = KeyManagerFactory.getInstance("SunX509");
                    KeyStore keystore = KeyStore.getInstance("PKCS12");
                    keystore.load(new ByteArrayInputStream(Connection.api.PfxFile), Connection.api.PfxFilePassword.toCharArray());
                    kmf.init(keystore, Connection.api.PfxFilePassword.toCharArray());
                    Enumeration<String> aliases = keystore.aliases();
                    PrivateKey key = null;
                    while (aliases.hasMoreElements()) {
                        String alias = aliases.nextElement();
                        key = (PrivateKey) keystore.getKey(alias, Connection.api.PfxFilePassword.toCharArray());
                    }
                    if (key == null)
