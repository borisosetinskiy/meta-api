        ...
        while (buf.gethasData()) {
            ...
            if ((hdr.Id & 0xFFFFFFFFL) == 0x1F9 || (hdr.Id & 0xFFFFFFFFL) == 0x1FA) {
                ...
                for (int i = 0; i < num; i++) {
                    ...
                    res.AccessesEx[i].AccessRec = buf.<AccessRecEx>CryptStruct(AccessRecEx.Size, new AccessRecEx());
                    res.AccessesEx[i].Addresses = buf.<AddressRecEx>CryptArray(AddressRecEx.Size, new AddressRecEx());
                }
                ...
                throw new UnsupportedOperationException("hdr.Id = 0x" + String.format("%X", hdr.Id & 0xFFFFFFFFL));
            }
            lst.add(res);
        }
