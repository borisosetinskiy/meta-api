...
public class Compressor {
        ...
        IntContainer dstLen = IntContainer.fromData(0);
        {
            ...
            if (len > 0) {
                ...
                if (dataAddress(p) == dataAddress(dst) && len <= 0xEE) {
                    (p = String8.nnc(p).shift(1)).set(-1, (byte) (len + 0x11));
                } else if (len <= 3) {
                    p.set(-2, (byte) (Byte.toUnsignedInt(p.get(-2)) | Byte.toUnsignedInt((byte) len)));
                } else if (len <= 0x12) {
                    (p = String8.nnc(p).shift(1)).set(-1, (byte) (len - 3));
                } else {
                    p = String8.nnc(p).shift(1);
                    int cnt = len - 0x12;
                    if (cnt > 0xFF) {
                        long lbl_U = 0x80808081L * (long) (cnt - 1);
                        int bl = (int) (lbl_U >>> 39);
                        p = String8.nnc(p).shift(bl);
                        for (int i = 0; i < bl; i++) {
                            cnt -= 0xFF;
                        }
                    }
                    (p = String8.nnc(p).shift(1)).set(-1, (byte) cnt);
                }
                for (int i = 0; i < len; i++) {
                    (p = String8.nnc(p).shift(1)).set(-1, (from = String8.nnc(from).shift(1)).get(-1));
                }
                p.toString();
            }
            (p = p.shift(1)).set(-1, (byte) 0x11);
            p = p.shift(2);
            dstLen.set((int) String8.dataDistance(dst, p));
        }
    ...
    public static int compressData(String8 src, int sizeSrc, String8 dst, IntContainer sizeDst) {
        ...
        String8 to = dst;
        String8 from = String8.nnc(src).shift(4);
        String8 start = src;
        String8 end = String8.nnc(src).shift(sizeSrc);
        int index = 0;
        ...
        for (int pos = 0; true; )
            switch (pos) {
                    ...
                    for (; true; pos = 0)
                        switch (pos) {
                            default:
                                ...
                                sizeSrc = (int) String8.dataDistance(ptr, from);
                                ...
                                buf[index] = String8.nnc(from);
                                int look = (int) String8.dataDistance(start, from);
                                if (look != 0) {
                                    if (look <= 3) {
                                        to.set(-2, (byte) (Byte.toUnsignedInt(to.get(-2)) | Byte.toUnsignedInt((byte) look)));
                                    } else if (look <= 0x12) {
                                        (to = String8.nnc(to).shift(1)).set(-1, (byte) (look - 3));
                                    } else {
                                        (to = String8.nnc(to).shift(1)).set(-1, (byte) 0);
                                        int cnt = look - 0x12;
                                        if (cnt > 0xFF) {
                                            long lbl_U = 0x80808081 * (long) (cnt - 1);
                                            int bl = (int) (lbl_U >>> 39);
                                            to = String8.nnc(to).shift(bl);
                                            for (int i = 0; i < bl; i++) {
                                                cnt -= 0xFF;
                                            }
                                        }
                                        (to = String8.nnc(to).shift(1)).set(-1, (byte) cnt);
                                    }
                                    for (int i = 0; i < look; i++) {
                                        (to = String8.nnc(to).shift(1)).set(-1, (start = String8.nnc(start).shift(1)).get(-1));
                                    }
                                }
                                from.Index += 3;
                                if ((from = String8.nnc(from).shift(1)).get(-1) == ptr.get(3) && (from = String8.nnc(from).shift(1)).get(-1) == ptr.get(4) && (from = String8.nnc(from).shift(1)).get(-1) == ptr.get(5) && (from = String8.nnc(from).shift(1)).get(-1) == ptr.get(6) && (from = String8.nnc(from).shift(1)).get(-1) == ptr.get(7) && (from = String8.nnc(from).shift(1)).get(-1) == ptr.get(8)) {
                                    ptr.Index += 9;
                                    ...
                                    look = (int) String8.dataDistance(start, from);
                                    if (sizeSrc <= 0x4000) {
                                        sizeSrc--;
                                        if (look <= 0x21) {
                                            (to = String8.nnc(to).shift(1)).set(-1, (byte) (look - 2 | 0x20));
                                        } else {
                                            (to = String8.nnc(to).shift(1)).set(-1, (byte) 0x20);
                                            int cnt = look - 0x21;
                                            if (cnt > 0xFF) {
                                                long lbl_U = 0x80808081L * (long) (cnt - 1);
                                                int bl = (int) (lbl_U >>> 39);
                                        ...
                                        sizeSrc -= 0x4000;
                                        if (look <= 9) {
                                            (to = String8.nnc(to).shift(1)).set(-1, (byte) (sizeSrc >> 11 & 8 | look - 2 | 0x10));
                                        } else {
                                            (to = String8.nnc(to).shift(1)).set(-1, (byte) (sizeSrc >> 11 & 8 | 0x10));
                                            int cnt = look - 9;
                                            if (cnt > 0xFF) {
                                                long lbl_U = 0x80808081L * (long) (cnt - 1);
                                                int bl = (int) (lbl_U >>> 39);
                                    ...
                                    (to = String8.nnc(to).shift(1)).set(-1, (byte) (sizeSrc << 2));
                                    (to = String8.nnc(to).shift(1)).set(-1, (byte) (sizeSrc >> 6));
                                } else {
                                    look = (int) String8.dataDistance(start, from = String8.nnc(from).shift(-1));
                                    if (sizeSrc <= 0x800) {
                                        sizeSrc--;
                                        (to = String8.nnc(to).shift(1)).set(-1, (byte) (look + 7 << 5 | (sizeSrc & 7) << 2));
                                        (to = String8.nnc(to).shift(1)).set(-1, (byte) (sizeSrc >> 3));
                ...
                case posCheckEnd:
                    pos = 0;
                    if (dataAddress(from) >= dataAddress(String8.nnc(end).shift(-0xD))) {
