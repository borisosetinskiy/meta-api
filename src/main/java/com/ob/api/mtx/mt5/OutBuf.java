...
class OutBuf {
    ...
    public final void CreateHeader(byte type, int id, boolean compressed) {
        byte[] hdr = new byte[9];
        hdr[0] = type; //type
        System.arraycopy(BitConverter.GetBytes(List.size()), 0, hdr, 1, BitConverter.GetBytes(List.size()).length); //size
        System.arraycopy(BitConverter.GetBytes((short) id), 0, hdr, 5, BitConverter.GetBytes((short) id).length); //ID
        if (compressed) {
            System.arraycopy(BitConverter.GetBytes((short) 3), 0, hdr, 7, BitConverter.GetBytes((short) 3).length);
        } else {
            System.arraycopy(BitConverter.GetBytes((short) 2), 0, hdr, 7, BitConverter.GetBytes((short) 2).length); //Flags PHF_COMPLETE
        }
...
//ORIGINAL LINE: var res = new byte[len*4];
        ...
        if (str != null) {
            System.arraycopy(str.getBytes(java.nio.charset.StandardCharsets.UTF_16LE), 0, res, 0, str.getBytes(java.nio.charset.StandardCharsets.UTF_16LE).length);
        }
        Add(res);
    }
