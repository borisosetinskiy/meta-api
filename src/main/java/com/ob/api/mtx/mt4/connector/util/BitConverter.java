...
public class BitConverter {
    ...
    public static short ToUInt16(byte[] bytes, int i) {
        return ToInt16(bytes, i);
    }

    public static long makeULongLong(long v) {
        return (long) (((v >> 32) & 0xffffffffL) | ((v & 0xffffffffL) << 32));
    }

    public static byte[] updateRandom(long id, int szBuf) {
        ...
        while (szBuf > 0) {
            ...
            int size = Math.min(szBuf, 8);
            byte[] rndBytes = longToBytesAndRevert(rnd);
            System.arraycopy(rndBytes, 0, buf, i, size);
            i += size;
            szBuf -= size;
        }
        return buf;
    ...
    private static byte[] longToBytesAndRevert(long x) {
